...
function hsyh() {
    ...
    var is_type = ''
    var live_date = random(1, 8), video_num = 1
    while (1) {
        try {
...
function hsdl() {
    ...
    var zh = '18072064352'
    var yzm = "3976"
    var is_verify = 2, is_send = false, is_login = false, send_money = false
    while (1) {
        try {
            if ((new Date()).getTime() - RT > stt * 60 * 1000) {
                TKB.xgsrizhi("时间够了退出")
...
function xgzl() {
    ...
    while (1) {
      ...
      try {
        ...
        if (text('编辑资料').exists() && text('保存').exists() && text('昵称').exists()) {
          if (a == 1) {
            click('点击更换头像')
            sleep(1000)
            click('从相册选择')
            sleep(4000)
            if (desc('显示根目录').exists() || text('最近').exists()) {
              TKB.xgsrizhi('选择第一张')
              click(250, 500)
              sleep(2000)
            }
          ...
          if (a == 2) {
            TKB.xgsrizhi('修改资料')
            if (text(nickname).exists()) {
              TKB.xgsrizhi("已经是该名字了")
              sleep(500)
            } else {
              if (id('com.ss.android.ugc.live:id/mc').exists()) {
                TKB.xgsrizhi('清除昵称')
                id('com.ss.android.ugc.live:id/mc').click()
                sleep(500)
                setText(0, nickname)
                sleep(1000)
              } else {
            ...
            TKB.xgsrizhi('设置性别')
            id('com.ss.android.ugc.live:id/aug').findOnce().parent().parent().click()
            sleep(500)
            click('男')
            sleep(2000)
