...
function getdyslip(idtz) {
    if (isget == 0) {
        ...
        if (uc.length > 0) {
            ...
            // TKB.set_log(j[0], j[1], jj[0], jj[1], jjj[0], jjj[1]);
            if (j[0] == j[1] && jj[0] == jj[1] && jjj[0] == jjj[1] || j[0] == jj[0] && j[0] == jjj[0]) {
                ...
                if (colors.isSimilar(colors.toString(ys), colors.toString(-16711679), 10, "diff")) {
                    // TKB.set_log("黑色底,没有进度条.视频长度短语15s")
                } else {
                    // TKB.set_log("视频时长超过15秒,开始判断,播放时间.")
                    jys = ys;
                    jy = y;
                    isget = 1;
                }
...
function isbftime(img, ysz, y, x) {
    // TKB.set_log("颜色值为: ", ysz, " ---- rbg: ", colors.toString(ysz))
    ...
    while (1) {
        ...
        try {
            ...
            //TKB.set_log("找色报错:", error, x, imgx)
        }

    }
    //TKB.set_log("坐标点:", x)
    //TKB.set_log("用时:", ostime(1) - t, "ms")
    return x
}
...
var zz = '美女|腿|滤镜|颜|jk|女生|美人|最美|颜值|漂亮|闺蜜|惊艳|长腿|高颜|可爱|性感|姐姐'
//新养号
function dyxyh() {
    TKB.set_log("抖音新养号")
    TKB.clear()
    ...
    while (1) {
        ...
        if (a == 0) {
            if (text("关注").exists() && text("推荐").exists() || id("com.ss.android.ugc.aweme:id/e9r").exists() && text("推荐").exists() || (id("com.ss.android.ugc.aweme:id/e9r").exists() && id("com.ss.android.ugc.aweme:id/a7m").exists())) {
                ...
                while (1) {
                    try {
                        if ((new Date()).getTime() - ssd > random(50, 55) * 60 * 1000) {
                            TKB.set_log("去看直播")
                            click(330, 1830)
                            sleep(2000)
                            a = 1
                        ...
                        z = iskeyopen(word)
                        if (text('点击进入直播间').exists() || text('直播中').exists() || z != true) {
                            TKB.set_log('下滑')
                            swipe(random(400, 600), 1600, random(400, 600), 400, random(500, 800));
                            try {
                                sleep(random(mini, max))
                            } catch (error) {
                                log(error)
                                sleep(200)
                            }
