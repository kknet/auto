 ...
 * @LastEditors: cainiao
 * @LastEditTime: 2020-10-09 13:07:33
...
this.dyc = {
    ...
    get_xihuannum: function () {  //获取当前界面喜欢数量  建议使用前 先调用get_biaoti函数
    ...
    get_pinglunum: function () {//获取当前界面评论数量  建议使用前 先调用get_biaoti函数
    ...
    get_pingluntap: function () {//执行当前界面 评论点击操作  建议使用前 先调用get_biaoti函数
        var uc = descMatches("评论.+，按钮").boundsInside(1, 1, this.x, this.y).findOnce()
        if (uc) {
            return tapobjex(uc);
        }
        return false;
    },
    get_dyslip: function () {//执行当前界面 进度条百分比  建议使用前 先调用get_biaoti函数
        if (this.slip.ui == 0) {
