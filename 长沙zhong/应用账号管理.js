...
// log(UpFixedData(name, appname, username, bianliang, data, datatype, title))
// log(UpAliveData(name, appname, username, bianliang, data, datatype, title))
// log(UpAliveData(name, appname, username, '喜欢', '喜欢', '2', '喜欢'))
//获取该页面文字
function getAllTexts(setting) {
    try {
        var setting = setting || {}
...
/**获取视频、评论库数据
 * @param {string} name:设备名
 * @param {string} imp_id:任务方案id
 * @param {string} store_name:资料库的标签名称
 */
function get_videoList(name, imp_id, store_name) {
    log('获取视频')
    var Tb = (new Date()).getTime()
    while (1) {
        try {
...
var contacts_type = '1,5'
log(get_phone('861065032044357', '玉林', '2', '0', [contacts_type]))