...
function result_name() {
    ...
    if (random(0, 1) == 0) {
        var xingname = xing[random(0, xing.length - 1)] + ming[random(0, ming.length - 1)]
    } else {
        var xingname = xing[random(0, xing.length - 1)] + ming[random(0, ming.length - 1)] + ming[random(0, ming.length - 1)]
    }
    return xingname
}
var phone_id
...
function tjlxr() {
    ...
    // var num = '2'
    // var city = '温州'
    // var contacts_type = [1,2,5]
    var num = random(mini, max)
    var phone_lists = [];
    var phoneid = 0
    var phone_name
    var x = get_phone(MEIname, city, num, phone_id, contacts_type)
    if (x != false || x != undefined || x != '') {
        for (var z = 0; z < num; z++) {
            ...
            if (x[z]['name'] != undefined) {
                phone_name = x[z]['name']
                TKB.set_log('phone_name' + phone_name)
            } else {
                phone_name = result_name()
                TKB.set_log('phone_name' + phone_name)
            }
            phone_lists.push([phone_name, Number(x[z]['phone'])])
        }
    } else {
        TKB.set_log('暂时没有联系人')
        toastLog('暂时没有联系人')
