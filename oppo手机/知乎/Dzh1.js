...
//-------------实际运行模块---------------
var run_time = 60 //该任务运行的时间
var fun //需要做的子任务

//知乎评论
var dqbaoming = "com.zhihu.android" //该项目包名
var qqb_pk = 'com.tencent.mtt' //QQ浏览器包名
var xmxh = "34" //项目序号
...
function zhxgzl() {
    ...
    var zz = TKB.getPersonInfo()
    if (zz == false || zz == '当前编号暂无绑定个人资料') {
        alert('素材获取错误')
        return false
    } else {
        var nickname = zz['nickname']
        var img = zz['avater']
        var jianjie = zz['signature']
        var sp = xztp(img)
        if (sp == false) {
            TKB.xgsrizhi("下载图片失败")
            return false
    ...
    while (1) {
        ...
        try {
            ...
            if (a == 1) {
                if (xs['是否修改名字'] == '是') {
                    // TKB.xgsrizhi('修改昵称')
                    if (id('com.zhihu.android:id/username').findOnce().text() == nickname) {
                        TKB.xgsrizhi("已经是该昵称了,昵称修改完成")
                        toastLog("昵称修改完成")
                        a = 2
                    } else if (id('com.zhihu.android:id/username').exists() && text('基本资料').exists()) {
                        TKB.xgsrizhi('输入昵称')
                        id('com.zhihu.android:id/username').findOnce().setText(nickname)
                        sleep(2000)
                    }
                } else {
                    a = 2
...
function zhdl() {
    ...
    // var zh = "18967726665"
    // var yzm = "12346"
    var is_verify = 3 //验证码获取最大次数
    var loginClick = 0 //一键登录次数
    var sex;
    var interestName; 
    try {
        if (fun == "养号") {
...
/** 知乎养号
 * xh_percentage  {number} 喜欢百分比,根据百分比对图文或视频进行喜欢
 * zt_percentage {number} 赞同百分比,根据百分比对图文或视频进行赞同
 * zt_numx {number} 赞同的数量，大于zt_numx才浏览
 */
function zhyh() {
    log('知乎养号')
    launch(dqbaoming)
    ...
    var RT = (new Date()).getTime()
    var TSD = (new Date()).getTime()
    var stt = random(90, 150) //脚本总运行时间
    var a = 0 //0观看推荐，1观看热榜
    var tem = 0 //首次打开点击推荐
    var temx = 0 //首次打开点击热榜
    var tj_time = random(30, 40) //推荐的浏览时间
    var rb_time = random(10, 30) //热榜的浏览时间
    var ii = 0 //ii下滑的次数，超过30返回
    var xh = 0
    var zt = 0
    var zt_num = 0
    if (xh == 0 || xh == '0') {
        xh_percentage = 10
        log('喜欢百分比' + xh_percentage)
    ...
    while (1) {
        ...
        if (a == 0) {
            ...
            while (1) {
                ...
                if ((desc('返回').exists() || id('com.zhihu.android:id/search').exists() || id('com.zhihu.android:id/search_btn').exists() || id('com.zhihu.android:id/more_container').exists() || text('搜索知乎内容').exists() || id('com.zhihu.android:id/more_container1').exists()) && className('android.widget.Image').depth('23').exists()) {
                    log("文章界面")
                    var dj = random(1, 100)
                    try {
                ...
                try {
                    ...
                    if (temx == 0) {
                        ...
                        if (text('全部榜单').exists() && text('我的榜单').exists()) {
                            ...
                            var cy = bd[random(1, bd.length - 1)]
                            log('点击' + cy)
                            click(cy, 0)
                            sleep(4000)
                            temx = 1
                    ...
                    if (id('com.zhihu.android:id/ranking_no').exists && text('推荐').exists() || id('com.zhihu.android:id/ranking_no_img').exists() && text('推荐').exists()) {
                        ...
                        for (var i = 0; i < random(0, 8); i++) {
                            swipe(random(400, 600), 1300, random(400, 600), random(300, 500), random(800, 2000))
                    ...
                    if ((id('com.zhihu.android:id/search').exists() || id('com.zhihu.android:id/search_btn').exists()) && (id('com.zhihu.android:id/more').exists() || id('com.zhihu.android:id/more_btn').exists()) && text('默认').exists()) {
                        try {
                            var ss = TKB.getAllTexts()
                            for (j = 0, len = ss.length; j < len; j++) {
                                if (ss[j].indexOf("赞同") != -1) {
...
/**知乎评论
 * comment_sentence {string} 评论的话术
 * comment_url {string} 评论作品链接
 */
function comment() {
        ...
        try {
            ...
            if (comment_url != undefined) {
                ...
                if (text('进入').exists()) {
                    ...
                    TKB.xgsrizhi("点击进入");
                    sleep(8000);
                    if (text("是否打开知乎App阅读全文").exists()) {
                        TKB.xgsrizhi("点击打开知乎作品链接");
                        click("确认")
                        sleep(5000)
                    } else if (text('知乎').exists() && text('打开').exists() && text('继续').exists()) {
                        TKB.xgsrizhi("继续浏览内容弹窗,点击打开");
                        click('打开')
                        sleep(8000)
                    } else {
                        TKB.xgsrizhi("点击坐标打开知乎作品链接");
                        click(749, 1122)
                        sleep(5000)
                    }
                }
                ...
                if ((desc('返回').exists() || id('com.zhihu.android:id/search').exists() || id('com.zhihu.android:id/search_btn').exists() || id('com.zhihu.android:id/more_container').exists() || text('搜索知乎内容').exists() || id('com.zhihu.android:id/more_container1').exists()) && className('android.widget.Image').depth('23').exists()) {
                    //随机滑动几次浏览
                    for (let i = 0; i < 5; i++) {
                        swipe(random(400, 500), 800, random(400, 500), 500, 1000)
                        sleep(2000)
                        swipe(random(400, 500), 500, random(400, 500), 800, 1000)
                        sleep(2000)
                    }
                    ...
                    while (true) {
                        ...
                        if ((new Date()).getTime() - pageStart < pageTime * 1000) {
                            TKB.xgsrizhi("作品内容滑动")
                            swipe(random(500, 600), 800, random(500, 600), 500, 1000)
                            sleep(2000)
                        } else {
                            break
                        }
                    }
                    if (textStartsWith("评论").exists() || descStartsWith("评论")) {
                        TKB.xgsrizhi("点击评论")
                        var ss = TKB.getAllTexts()
                        for (j = 0, len = ss.length; j < len; j++) {
        ...
        try {
            ...
            if (likeclick_url != undefined) {
                ...
                if ((desc('返回').exists() || id('com.zhihu.android:id/search').exists() || id('com.zhihu.android:id/search_btn').exists() || id('com.zhihu.android:id/more_container').exists() || text('搜索知乎内容').exists() || id('com.zhihu.android:id/more_container1').exists()) && className('android.widget.Image').depth('23').exists()) {
                    ...
                    if (textStartsWith("赞同").exists() || descStartsWith("评论")) {
                        TKB.xgsrizhi("点击赞同")
                        var ss = TKB.getAllTexts()
                        for (j = 0, len = ss.length; j < len; j++) {
                            if (ss[j].indexOf(" 赞同 ") != -1) {
...
/**知乎关注
 * focusonUrl {string} 个人主页链接
 * clickFocuson {number} 关注方式：0浏览用户第一个作品关注，1关注该链接用户的关注的其中一人
 * clickOther {number} 关注该链接用户的关注的第几人，从0开始
 */
function focusOn() {
    var startTime = (new Date()).getTime();
    var TSD = (new Date()).getTime()
        ...
        while (1) {
            try {
                ...
                if ((new Date()).getTime() - ssee > 60 * 1000) {
                    ...
                    if (renwux == false && (new Date()).getTime() - yunxing < run_time * 5 * 60 * 1000) {
                        TKB.xgsrizhi("继续知乎任务")
                    } else {
