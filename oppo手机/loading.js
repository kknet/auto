...
threads.start(function () {
    while (1) {
        ...
        sleep(200)
        clickable().text('始终允许').id('android:id/button1').findOnce() ? clickable().text('始终允许').id('android:id/button1').findOnce().click() : ''
...
function loading(package, modules, telNumber) {
    ...
    while(true){
        if (text('本机号码一键登录').exists()) {
            ...
            sleep(2000)
            text('本机号码一键登录').exists() ?  click('本机号码一键登录'): text('本机号码一键登录').findOnce().parent().parent().click()
            sleep(3000)
