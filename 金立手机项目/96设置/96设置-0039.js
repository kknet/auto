...
function timing() {
    ...
    var ifClick = true;
    var xs = TKB.zhid(sbip, run_id)
    var startTime = xs["开机时间"]
    var startHour = startTime.split(":")[0]
    var startMinutes = startTime.split(":")[1]
    var endTime = xs["关机时间"]
    var endtHour = endTime.split(":")[0]
    ...
    while (true) {
        try {
            ...
            if (ifClick == true) {
                if (text("设置").exists() && text("特色功能").exists()) {
                    while (!text("更多设置").exists()) {
                        swipe(500, 1700, 500, 400, 800)
                        sleep(2000)
                        if (text("更多设置").exists()) {
                            TKB.xgsrizhi("发现更多设置")
                ...
                if (text("向上导航").exists() && text("更多设置").exists()) {
                    while (!text("定时开关机").exists()) {
                        swipe(500, 1700, 500, 400, 800)
                        sleep(2000)
                        if (text("定时开关机").exists()) {
                            TKB.xgsrizhi("发现定时开关机")
                            break
                        }
                        sleep(2000)
                    }
                    if (text("定时开关机").exists()) {
                        click("定时开关机")
                        TKB.xgsrizhi("点击定时开关机")
                        sleep(3000)
                    }
                }
                ifClick = false;
            }
            if (text("定时开机").exists() && text("定时关机").exists()) {
                if (text("定时开机").exists()) {
                    click(543, 465)
                    TKB.xgsrizhi("点击定时开机")
                    sleep(3000)
                    if (text("设置定时开机").exists()) {
                        click(292, 307)
                        TKB.xgsrizhi("点击时间")
                        sleep(3000)
                        if (text("确定").exists() && text("取消").exists()) {
                            while (id("amigo:id/amigo_numberpicker_input").find()[0].text() != startHour) {
                                swipe(388, 1492, 388, 1388, 1000)
                                sleep(3000)
                                if (id("amigo:id/amigo_numberpicker_input").find()[0].text() == startHour) {
                                    TKB.xgsrizhi("发现时间")
                                    break
                                }
                            }
                            sleep(3000)
                            while (id("amigo:id/amigo_numberpicker_input").find()[1].text() != startMinutes) {
                                swipe(721, 1492, 721, 1388, 1000)
                                sleep(3000)
                                if (id("amigo:id/amigo_numberpicker_input").find()[1].text() == startMinutes) {
                                    TKB.xgsrizhi("发现时间")
                                    break
                                }
                            }
                ...
                if (text("定时关机").exists()) {
                    ...
                    TKB.xgsrizhi("点击定时关机")
                    sleep(3000)
                    if (text("设置定时关机").exists()) {
                        click(434, 294)
                        TKB.xgsrizhi("点击时间")
                        sleep(3000)
                        if (text("确定").exists() && text("取消").exists()) {
                            while (id("amigo:id/amigo_numberpicker_input").find()[0].text() != endtHour) {
                                swipe(388, 1492, 388, 1388, 1000)
                                sleep(3000)
                                if (id("amigo:id/amigo_numberpicker_input").find()[0].text() == endtHour) {
                                    TKB.xgsrizhi("发现时间")
                                    break
                                }
                            }
                            sleep(3000)
                            while (id("amigo:id/amigo_numberpicker_input").find()[1].text() != endMinutes) {
                                swipe(721, 1492, 721, 1388, 1000)
                                sleep(3000)
                                if (id("amigo:id/amigo_numberpicker_input").find()[1].text() == endMinutes) {
                                    TKB.xgsrizhi("发现时间")
                                    break
                                }
                            }
                ...
                if (id("com.mediatek.schpwronoff:id/timeDisplay").find()[0].text == startTime && id("com.mediatek.schpwronoff:id/timeDisplay").find()[1].text == endTime) {
                    toast("设置完成")
                    TKB.xgsrizhi("设置完成")
                    TKB.qinglihh()
                    return true
                ...
                TKB.xgsrizhi("找不到定时开关机第" + notInHome + "次");
                notInHome++;
                back();
                ifClick = true;
                sleep(2000);
