...
function dyptrenwu(){
    ...
    if (fwqrw == false){
        ...
        fzan = fwqrw["zan"]
        fpinglun = fwqrw["pinglun"]
        fbofang = fwqrw["bofnag"]
        fguanzhu = fwqrw["guanzhu"]
    }
    ...
    while(1){
        ...
        if (tem == 0){
            ...
            var yzan = dse["zan"]              //已做数量-赞
            var ypinglun = dse["pinglun"]      //已做数量-评论
            var ybofang = dse["bofang"]        //已做数量-播放
            var yguanzhu = dse["guanzhu"]      //已做数量-关注
            if (rwlx == 1){
...
//更新服务器
function gxfuwuqi(sbip, lx, user_id, yhbh){
    ...
    while (1){
        try {
            ...
            var url = "http://"+sbip+"/api.php/potal/meitu/uprecord?type="+ lx +"&user_id="+user_id+"&der_id="+yhbh+"&app_id=3&content=1"
            TKB.xgsrizhi("链接：" + url)
            var r = http.get(url);
            if( r.statusCode == 200){
                var r_obj = r.body.json();
...
// hqsbxx(sbip, user_id, yhbh)

// hqqjbl(sbip, user_id, yhbh)

// var aaa = {"a": "123", "b":"456", "c":"789"}

// alert(aaa.hasOwnProperty("123"))
...
rwurl = "https://v.douyin.com/JLV24qo/"
setClip(rwurl) 
