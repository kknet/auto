...
let checkpoint = require('/sdcard/killSP.js')
...
threads.start(function () {
    ...
    //     sleep(200)
    //     id('com.ss.android.ugc.aweme:id/xf').exists()?id('com.ss.android.ugc.aweme:id/xf').findOnce().click():''
...
function dydl() {
    ...
    closeandopen(dqbaoming, 10, 'single');
    sleep(15000)
    ...
    while (1) {
        try {
            ...
            if (text('本机号码一键登录').exists()) {
                ...
                sleep(2000)
                text('本机号码一键登录').exists() ? click('本机号码一键登录') : ''
                sleep(3000)
            ...
            if (text("首页").exists() && text("消息").exists()) {
                log('账号登录完成')
                return true;
            }
...
function main() {
    try {
        ...
        var dl = dydl()
        if (dl == true) {
            doaction(dqbaoming, res, randomProbal);
        }
        TKB.qinglihh()
