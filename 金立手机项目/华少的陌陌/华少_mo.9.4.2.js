...
var xiaomifeng = dialogs.singleChoice("请选择软件", ["小蜜蜂", "小霸王","手机号注册"], 0);
if (xiaomifeng == 0) {
...
function qinglimomo() {
    while (1) {
        openAppSetting(getPackageName("陌陌"));
...
function zhuce() {
    ...
    while (1) {
        try {
            ...
            if ((text('陌陌').exists() || text('陌陌').exists()) && (text(tongdaotext).exists() || text('一键授权').exists() || text(tongdaox).exists()) && sqnum == 0) {
            ...
            //直播间操作
            findeclick("id", "com.immomo.momo:id/ml_dialog_close_button", 800)
            if (id('com.immomo.momo:id/popup_phone_live_gesture_guide_tv').exists()) {
                back()
                sleep(800)
            }
            if (id('com.immomo.momo:id/live_iv_star_avatar').exists()) {
                back()
                sleep(800)
            }
            findeclick("text", "忽略", 800)
            ...
            if (findeclick("text", "搜索用户：471025670", 5000) && qz == 1) {
                toastLog("注册流程完成")
                back()
                sleep(500)
                back()
                sleep(2000)
                if (text('搜索用户：471025670').exists()) {
                    back()
                    sleep(2000)
                }
                return true
...
function shoujihaozhuce() {
    ...
    while (1) {
        try {
            ...
            if (findeclick("text", "搜索用户：807646435", 5000)) {
                toastLog("注册流程完成")
                back()
                sleep(500)
                back()
                sleep(2000)
                if (text('搜索用户：807646435').exists()) {
                    back()
                    sleep(2000)
                }
                return true
...
function getyzm() {
    var TS = (new Date()).getTime()
    var url = "http://106.54.66.160:6009/getsms&project=momo&user=huashao&pwd=huashao&phone=" + phone;
    log("获取验证码")
    while (true) {
        try {
            var res = http.get(url);
