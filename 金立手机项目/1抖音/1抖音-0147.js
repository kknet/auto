...
function iskeyopen(ss) {
    var res = false;
    //是否开启关键词检测
    //检测当前视频标题
    for (let i = 0; i < ss.length; i++) {
        try {
            var vediotext = id('a91').className('android.widget.TextView').findOnce() ? id('a91').className('android.widget.TextView').find()[1].text() : '';
        } catch (e) {
            log(e)
            continue;
        }
        // log('标题' + vediotext)
        if (vediotext.indexOf(ss[i].replace(/[\r\n]/g, "")) != -1) {
            res = true;
            log('关键词通过')
        }
    }
                ...
                while (1) {
                    ...
                    if (tem == 0) {
                        ...
                        if ((new Date()).getTime() - ssd > 40 * 60 * 1000 && ll < 5) {
                            ...
                            try {
                                if ((new Date()).getTime() - xht > 30 * 1000) {
                                    TKB.xgsrizhi('下滑1')
                                    swipe(500, 1600, 600, 400, 1200);
                                    xht = (new Date()).getTime()
                                } else {
                                    z = iskeyopen(ss)
                                }
    ...
    while (true) {
        try {
            ...
            if (text("关注").exists() && text("消息").exists() && text("我").exists()) {
                ...
                sleep(5000)
                text("暂无关注人动态").exists() ? (click("推荐"), log("点击推荐")) : ''
                sleep(5000)
