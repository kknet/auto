            ...
            try {
                for (j = 0, len = ss.length; j < len; j++) {
                    if (ss[j].indexOf("喜欢 ") != -1) {
                        ...
                        djsl = st[1]
                        log("点赞数量：" + djsl)
                        var data = 'jianjie3',datatype = '2',title = '简介2',bianliang = '简介'
                        TKB.UpAliveData(sbip, user_id, yhbh, app_id, "xihuan", djsl)
                    ...
                    if (ss[j].indexOf("作品 ") != -1) {
                        ...
                        zpsl = st[1]
                        log("作品数量：" + zpsl)
                        TKB.UpAliveData(sbip, user_id, yhbh, app_id, "shipin", zpsl)
                        TKB.UpAliveData(name, appname, username, '作品', data, datatype, title)
                    }
                ...
                if (dybf != 0) {
                    ...
                    TKB.UpAliveData(name, appname, username, bianliang, data, datatype, title)
                }
