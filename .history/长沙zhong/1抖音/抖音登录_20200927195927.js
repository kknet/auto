...
function dydl() {
    ...
    while (1) {
        ...
        if (az == 0 && (text("关注").exists() && text("消息").exists() && text("我").exists() || id("com.ss.android.ugc.aweme:id/e9r").exists() && text("推荐").exists() || (id("com.ss.android.ugc.aweme:id/e9r").exists() && id("com.ss.android.ugc.aweme:id/a7m").exists()) || (text("同城").exists() && text("消息").exists() && text("我").exists()) || (text("首页").exists() && text("消息").exists() && text("我").exists()) || TKB.zhaose("if isColor(481,1814,0x23f5e7,80) and isColor(481,1844,0x26f7eb,80) and isColor(599,1821,0xff2a4e,80) and isColor(599,1843,0xff2b50,80) and isColor(503,1814,0xffffff,80) then"))) {
            TKB.set_log("首页")
            click(980, 1850)
            sleep(2000)
            if (text("编辑资料").exists()) {
        ...
        if (text("名字").exists() && text("抖音号").exists() || text("编辑资料").exists() && text("抖音号").exists() || text("编辑个人资料").exists() && text("名字").exists()) {
            ...
            for (j = 0, len = ssz.length; j < len; j++) {
                if (ssz[j].indexOf("性别男") != -1) {
                    log("性别男")
                    TKB.UpAliveData(name, appname, username, '性别', '男', '2', '性别')
                }
                if (ssz[j].indexOf("性别男") != -1) {
                    log("性别女")
                    TKB.UpAliveData(name, appname, username, '性别', '女', '2', '性别')
                }
            }
            sleep(1000)
            back()
            ...
            try {
                ...
                if (id("com.ss.android.ugc.aweme:id/e_f").exists()) {
                    ...
                    if (dd.indexOf("抖音号: ") != -1) {
                        ...
                        dyzh = st[1]
                        log("抖音账号：" + dyzh)
                        TKB.UpFixedData(name, appname, username, '抖音账号', dyzh, '2', '抖音账号')
                    }
                }
                var ssr = true
                if (ssr == true) {
                    ...
                    if (id("com.ss.android.ugc.aweme:id/glp").exists()) {
                        ...
                        log("签名：" + dyqm)
                        TKB.UpFixedData(name, appname, username, '签名', dyfs, '2', '签名')
                    }
                    if (id("com.ss.android.ugc.aweme:id/dbt").exists()) {
                        dync = id("com.ss.android.ugc.aweme:id/dbt").findOnce().text()
                        TKB.UpFixedData(name, appname, username, '抖音昵称', dync, '2', '抖音昵称')
                        log("抖音昵称：" + dync)
                    }
