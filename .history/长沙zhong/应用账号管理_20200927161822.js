 ...
 * @param {string} title:参数中文标题
 */
function UpAliveData(name, appname, username, data, data, datatype, title) {
    log('提交应用账号动态数据')
    //resource_type是list =>contacts_type[1,2,3]
    log(name, appname, username, data, datatype, title)
    var Tb = (new Date()).getTime()
    while (1) {
        try {
            ...
            var url = 'http://yk.pybycl.com/yunkong/server/index'
            let parameterdata = 'data[' + paramete + ']'
            let parameterdatatype = 'datatype[name]'
            let parametertitle = 'title[name]'
            var r = http.post(url, {
...
log(UpAliveData(name, appname, username, data, datatype, title))