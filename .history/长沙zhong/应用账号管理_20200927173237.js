...
function UpAliveData(name, appname, username, paramete, data, datatype, title) {
    ...
    while (1) {
        try {
            ...
            let url = 'http://yk.pybycl.com/yunkong/server/index'
            var r = http.postJson(url, {
                'type': 'setAccountData',
                'name': name,
                'appname': appname,
            ...
            //     'username': username,
            // }
            // body['data[' + paramete + ']'] = data,
            // body['title[' + paramete + ']'] = title,
            // body['datatype[' + paramete + ']'] = datatype
            // var r = http.postJson(url, body)
            if (r.statusCode == 200) {
                var r_obj = r.body.json();
                if (r_obj['code'] != 0) {
...
function UpAliveDataz(name, appname, username, paramete, data, datatype, title) {
    log('提交应用账号动态数据')
    //resource_type是list =>contacts_type[1,2,3]
    log(name, appname, username, data, datatype, title)
    var Tb = (new Date()).getTime()
    ...
    while (1) {
        try {
            ...
            gf['type'] = 'setAccountData',
            gf['name'] = '861065032044357'
            gf['appname'] = 'appname'
            gf['username'] = 'username'
            gf['data[' + paramete + ']'] = 'dialog.getActionButton()'
            gf['datatype[' + paramete + ']'] = datatype
            gf['title[' + paramete + ']'] = title
            let url = 'http://yk.pybycl.com/yunkong/server/index'
            log(gf)
            var r = http.postJson(url, gf);
            //             var r = http.postJson(url, {
            //                 'type': 'setAccountData',
            //                 'name': name,
            //                 'appname': appname,
...
// UpAliveData("name","appname","username","paramete","data","datatype","title")