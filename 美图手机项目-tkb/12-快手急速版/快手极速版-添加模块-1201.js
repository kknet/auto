...
var dqbaoming = "com.kuaishou.nebula"   //该项目包名  版本：2.3.0.231
var xmxh = "98"               //项目序号
...
function wxsm(){
    ...
    while(1){
        ...
        if(id("com.tencent.mm:id/drq").exists() && id("com.tencent.mm:id/f2z").exists() || text("轻触照亮").exists() && id("com.tencent.mm:id/f2z").exists()){
            TKB.xgsrizhi("扫码界面")
            click(930, 1700)   
            sleep(2000)
        }
        if(text("所有图片").exists() && id("com.tencent.mm:id/dn").exists()){
            TKB.xgsrizhi("选择照片界面")
            if (id("com.tencent.mm:id/evu").exists()){
                TKB.xgsrizhi("点击照片")
            }else{
                toastLog("可能没照片")
                sleep(2000)
        ...
        if(text("下载").exists() && text("快手极速版").exists()  && id("com.tencent.mm:id/bcd").exists()){
            TKB.xgsrizhi("点击下载")
            click(500, 1000)   
            sleep(2000)
        }

        if(text("取消下载").exists() && text("快手极速版").exists()  && id("com.tencent.mm:id/bcd").exists()){
            TKB.xgsrizhi("取消下载")
            toastLog("正在下载中")
            sleep(5000)
        }
