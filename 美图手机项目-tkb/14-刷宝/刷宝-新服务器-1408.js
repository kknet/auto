...
function sbdl(){
    ...
    while(1){
        ...
        if(text("我的钱包").exists() && id("com.jm.video:id/iv_setting").exists() || text("我的视频").exists() && id("com.jm.video:id/iv_head").exists()){
            ...
            toastLog("登陆成功")
            tem = 1
            if (dds < 2 && id("com.jm.video:id/tv_mine_money").exists()){
                sleep(3000)
                try{
                    var ss = id("com.jm.video:id/tv_mine_money").findOnce().text()
                    if (ss != 0 && ss != "0" && ss != ""){
                        TKB.upjine(sbip, yhname, yhbh, "shuabao", ss)
                        dds = 10
                    }
                }catch(error) {
                    sleep(1001)
    ...
    while(1){
        ...
        if(text("我的钱包").exists() && id("com.jm.video:id/iv_setting").exists() || text("我的视频").exists() && id("com.jm.video:id/iv_head").exists()){
            ...
            if (dds < 2 && id("com.jm.video:id/tv_mine_money").exists()){
                ...
                try{
                    // var ss = id("com.jm.video:id/tv_mine_money").findOnce().text()
                    ss = id("com.jm.video:id/tv_mine_money").findOnce().bounds()
                    TKB.xgsrizhi(ss)
                    click(ss.centerX(), ss.centerY());
                    sleep(2000)
                }catch(error) {
        ...
        if (text("我的钱包").exists() && desc("立即提现").exists() || desc("收益说明").exists() && desc("立即提现").exists()){
            TKB.xgsrizhi("立即提现")
            try{
                ss = desc("立即提现").findOnce().bounds()
                TKB.xgsrizhi(ss)
                click(ss.centerX(), ss.centerY());
                sleep(2000)
            }catch(error) {
        ...
        if (desc("取消").exists() && desc("给好评").exists()){
            TKB.xgsrizhi("提现成功")
            try{
                ss = desc("取消").findOnce().bounds()
                TKB.xgsrizhi(ss)
                click(ss.centerX(), ss.centerY());
                sleep(2000)
            }catch(error) {
        ...
        if (desc("当前余额").exists() && desc("我的元宝").exists() || desc("提现").exists() && desc("兑现").exists()){
            TKB.xgsrizhi("任务界面")
            try{
                var ss = desc("提现").findOnce().bounds();
                TKB.xgsrizhi(ss)
                click(ss.centerX(), ss.centerY());
                sleep(2000)
            }catch(error){
