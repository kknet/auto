...
function zonghe(){
    ...
    if(text("继续播放").exists() && text("取消播放").exists()){
        TKB.xgsrizhi("继续播放")
        click("继续播放")
        sleep(2000)
    }
    if(text("恭喜获得").exists() && text("金币").exists()){
        TKB.xgsrizhi("恭喜获得")
        click(520, 1500)
        sleep(2000)
    }
    if(text("继续观看").exists() && text("我要免流量").exists()){
        TKB.xgsrizhi("继续观看")
        click("继续观看")
        sleep(2000)
    }
                ...
                if(text("推荐").exists() && id("com.jifen.qukan:id/air").exists() || id("com.jifen.qukan:id/bur").exists()){
                    ...
                    try{
                        if (text("领取").exists()){
                            ...
                            // click("领取")
                            try{
                                var ss = text("领取").findOnce().bounds()
                                TKB.xgsrizhi(ss)
                                if (ss.centerY() < 300){
                                    click(ss.centerX(), ss.centerY());
                                    sleep(2000)
                ...
                if(id("com.jifen.qukan:id/qo").exists() && id("com.jifen.qukan:id/amf").exists() || id("com.jifen.qukan:id/am7").exists() && id("com.jifen.qukan:id/qj").exists()){
                    ...
                    while(1){
                        if (id("com.jifen.qukan:id/aww").exists()){
                            try{
                                var ss = id("com.jifen.qukan:id/aww").findOnce().bounds()
                                TKB.xgsrizhi(ss)
                                click(ss.centerX(), ss.centerY());
                                sleep(2000)
                            }catch(error) {
...
function qttdl(){
    ...
    while(1){
        ...
        if(text("我要提现").exists() && id("com.jifen.qukan:id/buy").exists() || desc("1元").exists() && id("com.jifen.qukan:id/buy").exists()){
            TKB.xgsrizhi("我要提现")
            back()
            sleep(2000)
        }
        ...
        if ((new Date()).getTime() - fdd > 60*1000){
            if (TKB.zhaose("if isColor(198,572,0xea4f39,80) and isColor(553,539,0xffdfb6,80) and isColor(492,1482,0xffaf3a,80) and isColor(941,1106,0xe52331,80) then")){
                TKB.xgsrizhi("新人有礼")
                click(551,1429)
                sleep(2000)
            }
            fdd = (new Date()).getTime()
        }
        zonghe()
    }
}
...
function qttzhixing(){
    try {
        ...
        _THREADSSxx = threads.start(function (){
            ...
            if (wb == true){
                qttyh()
                // var rw = TKB.huoqufzrw(sbip, yhname, yhbh, "qutoutiao")
                // if (rw == "养号"){
                //     sbyh()
                // }
            }
