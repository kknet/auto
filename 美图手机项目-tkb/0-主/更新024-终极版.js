...
//获取任务
function huoqurenwu(sbip, yhname, yhbh){
    xgsrizhi("获取任务")
...
function qdtime(sbip, yhname, yhbh){
    ...
    while (1){
        try {
            if ((new Date()).getTime() - Tb > 3*60*1000){
                xgsrizhi("超时-启动时间上传失败")
                toastLog("启动时间上传失败")
                return false
            }
...
function zhu(){
    ...
    while(1){
        try{
            ...
            if ((new Date()).getTime() - TJH > 4*60*1000){
            ...
            if (runzt == "tingzhi"){
                xzmk()
                xjihuoxt(ips, yhuser, bianhao)
                var renwu = huoqurenwu(ips, yhuser, bianhao)
