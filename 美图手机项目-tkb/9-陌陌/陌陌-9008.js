...
function zonghe(){
    ...
    if(id("com.immomo.momo:id/auth_module_dialog_iv_close").exists() && text("绑定手机").exists()){
        log("绑定手机")
        try {
            var ss = id("com.immomo.momo:id/auth_module_dialog_iv_close").findOnce().bounds();
            log(ss)
...
function mmyh(){
    ...
    while(1){
        ...
        if (id("com.immomo.momo:id/mk_dialog_close_button").exists()){
            log("直播弹出的提示")
            try{
                var ss = id("com.immomo.momo:id/mk_dialog_close_button").findOnce().bounds();
                log(ss)
                click(ss.centerX(), ss.centerY());
                sleep(2000)
            }catch(error) {
