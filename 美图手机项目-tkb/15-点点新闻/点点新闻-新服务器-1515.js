...
function ddyh(){
    ...
    while(1){
                    ...
                    if(id("com.yingliang.clicknews:id/iv_reward").exists()){
                        ...
                        try{
                            if (id("com.yingliang.clicknews:id/iv_reward").exists()){
                                var ss = id("com.yingliang.clicknews:id/iv_reward").findOnce().bounds();
                                TKB.xgsrizhi(ss)
                                if (ss.centerY() < 1890 && ss.centerY() > 305){
                                    click(ss.centerX(), ss.centerY());
                        ...
                        try{
                            var ssdd = TKB.getAllTexts()
                            for(var ii = 0; ii < ssdd.length; ii++){
                                if (ssdd[ii].length > 25){
                                    TKB.xgsrizhi("该字符串的长度为："+ ssdd[ii].length)
                                    TKB.xgsrizhi("该字符串的长度为："+ ssdd[ii])
                                    if (text(ssdd[ii]).exists()){
                                        TKB.xgsrizhi("点击进入新闻")
                                        var ssw = text(ssdd[ii]).findOnce().bounds();
                                        TKB.xgsrizhi(ssw)
                                        if (ssw.centerY() > 310 && ssw.centerY() < 1890){
                                            click(ssw.centerX(), ssw.centerY());
            ...
            if(id("com.yingliang.clicknews:id/tv_share").exists() && id("com.yingliang.clicknews:id/back").exists() || id("com.yingliang.clicknews:id/imageView1").exists() && id("com.yingliang.clicknews:id/frameLayout1").exists()){
                ...
                while(1){
                    if ((new Date()).getTime() - bbs > zbt*1000){
                        ...
                        sleep(random(2000, 5000))
                        toastLog("浏览新闻内容")
                        swipe(533,random(1800, 1000), 557,random(800, 400),random(1000, 2000))
                        sleep(random(2000, 5000))
                    }
                    if (text("大家都在看").exists()){
                        try{
                            toastLog("大家都在看")
                            var ss = text("大家都在看").findOnce().bounds();
                            TKB.xgsrizhi(ss)
                            click(ss.centerX(), ss.centerY());
                            if (ss.centerY() > 400 && ss.centerY() < 1900){
                                sleep(random(3000, 5000))
                                back()
                                sleep(2500)
                                break
        ...
        if(id("com.yingliang.clicknews:id/tt_video_ad_close").exists() && id("com.yingliang.clicknews:id/tt_video_ad_close_layout").exists() || text("com.yingliang.clicknews:id/tt_video_ad_close").exists() && desc("下载阅读").exists() || id("com.yingliang.clicknews:id/tt_video_ad_close").exists() && id("com.yingliang.clicknews:id/tt_reward_ad_download").exists()){
            TKB.xgsrizhi("看完广告了")
            try{
                if (id("com.yingliang.clicknews:id/iv_reward").exists()){
                    var ss = id("com.yingliang.clicknews:id/iv_reward").findOnce().bounds();
            ...
            if(id("com.yingliang.clicknews:id/tt_video_ad_close").exists()){
                TKB.xgsrizhi("看完广告了2")
                try{
                    if (id("com.yingliang.clicknews:id/tt_video_ad_close").exists()){
                        var ss = id("com.yingliang.clicknews:id/tt_video_ad_close").findOnce().bounds();
                        TKB.xgsrizhi(ss)
                        click(ss.centerX(), ss.centerY());
                        sleep(random(1000, 2000))
                    }
    ...
    while(1){
        ...
        if(text("登录/注册").exists()){
            ...
            if(text("邀请赚钱").exists() && id("com.yingliang.clicknews:id/iv_avatar").exists() || id("com.yingliang.clicknews:id/fl_name").exists() && id("com.yingliang.clicknews:id/tv_id").exists()){
                TKB.xgsrizhi("登录成功")
                toastLog("登录成功")
                if (dds < 2 && id("com.yingliang.clicknews:id/tv_history_money").exists()){
                    sleep(3000)
                    try{
                        var ss = id("com.yingliang.clicknews:id/tv_history_money").findOnce().text()
                        if (ss != "0" && ss != 0 && ss != ""){
                            TKB.upjine(sbip, yhname, yhbh, "diandianxinwen", ss)
                            log("我是金额" + ss)
                            dds = 10
                        }
                    }catch(error) {
...
function ddsjdl(){
    ...
    while(1){
        ...
        if(text("登录拆红包").exists() && text("登录即同意").exists() || text("登录拆红包").exists() && id("com.yingliang.clicknews:id/iv_icon_mobile").exists()){
            TKB.xgsrizhi("输入手机号码界面")
            setText([0], zh)
            sleep(500)
...
//点点新闻登录提现
function dddltx(){
    TKB.xgsrizhi("点点新闻登录提现")
    launch("com.yingliang.clicknews")
    sleep(4000)
    var RT = (new Date()).getTime()
    var stt = random(15, 16)
    var TSD = (new Date()).getTime()
    var ddn = (new Date()).getTime()
    var tem = 0
    var cs = 0
    ...
    while(1){
        ...
        if(text("登录/注册").exists()){
            ...
            if(text("金币").exists() && text("提现兑换").exists() || text("邀请赚钱").exists() && id("com.yingliang.clicknews:id/iv_avatar").exists() || id("com.yingliang.clicknews:id/fl_name").exists() && id("com.yingliang.clicknews:id/tv_id").exists()){
                TKB.xgsrizhi("登录成功")
                if (text("提现兑换").exists()){
                    sleep(3000)
                    try{
        ...
        if(text("微信提现").exists() && text("提现").exists() || text("微信提现").exists() && text("兑换记录").exists()){
            TKB.xgsrizhi("微信提现")
            click("微信提现")
            sleep(3000)
        }
        if(text("微信1元提现，秒到账").exists() && text("微信30元无门槛提现").exists()){
            TKB.xgsrizhi("选择金额提现界面")
            sleep(2000)
        }

        if(text("余额：").exists() && text("点击兑换").exists() || text("· 商品详情 ·").exists() && text("点击兑换").exists()){
            TKB.xgsrizhi("点击兑换")
            dds = dds + 1
            try{
                var ss = text("点击兑换").findOnce().bounds();
                click(ss.centerX(), ss.centerY());
                sleep(2000)
            }catch(error) {
                sleep(1001)
        ...
        if(text("订单确认").exists() && text("确认兑换").exists() || text("确认兑换").exists() && text("收货信息").exists()){
            TKB.xgsrizhi("确认兑换")  //点击微信授权
            try{
                if (text("点击微信授权").exists()){
                    TKB.xgsrizhi("点击微信授权")
                    var ss = text("点击微信授权").findOnce().bounds();
                    click(ss.centerX(), ss.centerY());
                    sleep(2000)
                }else{
                    var ss = text("确认兑换").findOnce().bounds();
                    click(ss.centerX(), ss.centerY());
                    sleep(2000)
                }
            }catch(error) {
        ...
        if(text("确认接收").exists() && text("取消").exists()){
            TKB.xgsrizhi("确认接收") 
            click("确认接收") 
            sleep(4000)
        }
