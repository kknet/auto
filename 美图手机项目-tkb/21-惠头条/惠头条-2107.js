...
var dqbaoming = "com.cashtoutiao"   //该项目包名  版本 2.100
var xmxh = "21"               //项目序号
...
function zonghe(){
    ...
    if(desc("货到付款").exists() && desc("全国包邮").exists()){
        TKB.xgsrizhi("全国包邮")
        back()
        sleep(3000)
    }
    if(text("温馨提示").exists() && text("取消").exists()){
        TKB.xgsrizhi("温馨提示")
        click("取消")
        sleep(2000)
    }
    if(text("继续播放").exists() && text("退出播放").exists()){
        TKB.xgsrizhi("继续播放")
        click("继续播放")
        sleep(2000)
    }
    ...
    if(id("com.cashtoutiao:id/img_close").exists()){
        TKB.xgsrizhi("关闭提示")
        try{
            ss = id("com.cashtoutiao:id/img_close").findOnce().bounds()
    ...
    if(id("com.cashtoutiao:id/tt_video_ad_close").exists()){
        TKB.xgsrizhi("关闭红包广告")
        try{
            ss = id("com.cashtoutiao:id/tt_video_ad_close").findOnce().bounds()
            click(ss.centerX(), ss.centerY())
            sleep(2000)
        }catch(error) {
            sleep(1001)
...
function httyh(){
    ...
    // TKB.qinglihh()
    sleep(2000)
    launch("com.cashtoutiao")
    var RT = (new Date()).getTime()
    var stt = random(40, 60)
    ...
    while(1){
        ...
        if(text("推荐").exists() && text("视频").exists() || text("任务中心").exists() && text("视频").exists()){
            ...
            if(id("com.cashtoutiao:id/iv_news_shielding").exists() || id("com.cashtoutiao:id/tv_news_src").exists()){
                log("新闻界面")
                try{
        ...
        if(id("com.cashtoutiao:id/iv_back").exists() && id("com.cashtoutiao:id/iv_right").exists() || text("评论").exists() && text("分享").exists()){
            TKB.xgsrizhi("浏览新闻界面")
            ...
            for(j = 0,len=dsv; j < len; j++){
                if(desc("查看全文，奖励更多").exists()){
                    ...
                    try{
                        var ss = desc("查看全文，奖励更多").findOnce().bounds()
                        TKB.xgsrizhi(ss.centerX())
                        TKB.xgsrizhi(ss.centerY())
                        if (ss.centerY() > 300 && ss.centerY() < 1800){
                            click(ss.centerX(), ss.centerY());
                        }
                        sleep(2000)
                    }catch(error) {
                    ...
                    if(desc("展开全文").exists()){
                        TKB.xgsrizhi("展开全文")
                        try{
                            var ss = desc("展开全文").findOnce().bounds()
                            TKB.xgsrizhi(ss.centerX())
                            TKB.xgsrizhi(ss.centerY())
                            if (ss.centerY() > 300 && ss.centerY() < 1800){
                                click(ss.centerX(), ss.centerY());
        ...
        if(id("com.cashtoutiao:id/toolbar_title_layout").exists() && id("com.cashtoutiao:id/toolbar_title").exists()){
            TKB.xgsrizhi("广告界面")
            back()
            sleep(3000)
        }
        if (text("温馨提示").exists() && text("仍要退出").exists()){
            TKB.xgsrizhi("仍要退出")
            click("仍要退出")
            sleep(3000)
        }
...
function httdl(){
    ...
    while(1){
        ...
        if(text("我的").exists() && text("视频").exists() || text("任务中心").exists() && text("视频").exists()){
            TKB.xgsrizhi("首页")
            click(970, 1960)
            sleep(2000)
        }
        ...
        if(id("com.cashtoutiao:id/imageView_header").exists() && text("历史总金币").exists() || text("兑换提现").exists() && text("我的订单").exists()){
            TKB.xgsrizhi("登录成功")
            return true
        }
        ...
        while(1){
            try{
                ...
                if ((new Date()).getTime() - ssee > 60 *1000){
                    ...
                    if (renwu == true){
                        TKB.xgsrizhi("继续惠头条任务")
                    }else{
                        TKB.xgsrizhi("服务器上没有惠头条任务")
                        TKB.qinglihh()
                        sleep(2000)
...
function huittzhixing(){
    try {
        bofangyy()
        TKB.xgsrizhi("执行惠头条")
        _THREADSSxx = threads.start(function (){
            var baom = getPackageName("惠头条")
            if (baom == null){
                log("未安装惠头条")
                var bbd = TKB.wdjxiazai("惠头条", "4.1.9.0")
                if (bbd == false){
                    TKB.xgsrizhi("安装惠头条点不成功")
                    _THREADSS.interrupt()
                    TKB.cunqusj("jiaoben","tingzhi")
                    java.lang.System.exit(0);
                }
